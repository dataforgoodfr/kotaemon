...
COLLECTION_ID = 1 # collection id is the integer at the end of the collection 'index_[ID] '
USER_ID = '25**********************' # ! => Open the Kotaemon app ... go on a random page... see the logs to retrieve the USER ID !
...
class IndexingPipelineShortCut(IndexPipeline):
    ...
    def handle_docs(self, docs, file_id, file_path, article_metadata: dict | None = None) -> int:
        s_time = time.time()
        ...
        try:
            ...
            metadatas_entire_doc = self.concat__metadatas_layer(metadatas_base=llm_metadatas,
                                                          metadatas_root=METADATA_BASE)
            
            # Enrich metadatas with other functional metadatas
            metadatas_entire_doc = self.concat__metadatas_layer(metadatas_base={'doc_type': 'entire_doc'},
                                                          metadatas_root=metadatas_entire_doc)
            
            # Enrich metadatas with article metadata if provided
            if article_metadata:
                metadatas_entire_doc = self.concat__metadatas_layer(metadatas_base=article_metadata,
                                                          metadatas_root=metadatas_entire_doc)
    ...
    def run_one_file(
        ...
        try:
            ...
            # ---- MOCK ADDITIONNAL METADATA EXTRACTED (BASE) -----------
